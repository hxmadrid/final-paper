...
star$Species <- factor(star$Species, levels = c("Archaster typicus", 
                                            ...
                                            "Linckia laevigata"))
star$Site <- factor(star$Site, levels = c("Intermediate","Pristine",
                                          "Polluted"))
...
glmm.null <- lmer(RB.sec ~ (1|Date), REML = FALSE, data = star)

summary(glmm.null)

glmm.all <- glmmTMB(RB.sec ~ Species + Site + (1|Date), family = "tweedie",
                    data = star)

glmm.site <- glmmTMB(RB.sec ~ Site, family = "tweedie",
                    data = star)
