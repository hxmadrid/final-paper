...
star <- data %>%
  dplyr::select(ID, Site, Species, RB, Abs15.R1, Abs15.R2, Abs15.R3,
         Abs30.R1, Abs30.R2, Abs30.R3,
         Abs60.R1, Abs60.R2, Abs60.R3,
         Abs90.R1, Abs90.R2, Abs90.R3, Date, Location) %>%
...
### 6.a.i model_RB between sites across species
```{r}

## converts to factor
star$Date.Num <- factor(star$Date, levels = c("03/20/2024","03/12/2024", "03/17/2024"),
                    labels = c("One", "Two", "Three"))
star$ID <- factor(star$ID)
star$Location <- factor(star$Location)

#### trial ####

id1 <-glmmTMB(RB.sec ~ Site + (1|Date.Num) + (1|Species) + (1|ID), 
            family="tweedie",data = star)

id2 <- glmmTMB(RB.sec ~ Site + (1|Species) + (1|ID), 
            family="tweedie",data = star)

id3 <- glmmTMB(RB.sec ~ Site + (1|ID), 
            family="tweedie",data = pn)

id4 <- glmmTMB(RB.sec ~ 1 + (1|ID), 
            family="tweedie",data = pn)

#### models ####

##  RB between sites across species
ss1 <-glmmTMB(RB.sec ~ Site + (1|Location) + (1|Species), 
            family="tweedie",data = star)

ss0 <-glmmTMB(RB.sec ~ 1 + (1|Location) + (1|Species), 
            family="tweedie", data = star)
...
#### post-hoc ####
emmeans(ss1, list(pairwise ~ Site), adjust = "tukey")

```

### 6.a.ii. model_RB between sites per species
```{r}
##  RB between sites per species
sps.with <-glmmTMB(RB.sec ~ Site + (1|Location), family="tweedie",data = star)

sps.none <-glmmTMB(RB.sec ~ 1 + (1|Location), family="tweedie", data = star)
...
## converts to factor
at$Species <- factor(at$Species)
at$Site <- factor(at$Site)

at$Date.Num <- factor(at$Date, levels = c("03/20/2024","03/12/2024", "03/17/2024"),
                    labels = c("One", "Two", "Three"))

#### model ####
at1 <-glmmTMB(RB.sec ~ Site + (1|Location), data = at)

at0 <-glmmTMB(RB.sec ~ 1 + (1|Location), data = at)
...
#### post-hoc ####
emmeans(at1, list(pairwise ~ Site), adjust = "tukey")
...
## converts to factor
pn$Species <- factor(pn$Species)
pn$Site <- factor(pn$Site)
pn$Location <- factor(pn$Location)

pn$Date.Num <- factor(pn$Date, levels = c("03/20/2024","03/12/2024", "03/17/2024"),
                    labels = c("One", "Two", "Three"))

#### model ####
pn1 <-glmmTMB(RB.sec ~ Site + (1|Location), data = pn)

pn0 <-glmmTMB(RB.sec ~ 1 + (1|Location), data = pn)

##  compare models 
anova(pn1, pn0)

#### post-hoc ####
emmeans(pn1, list(pairwise ~ Site), adjust = "tukey")
...
## converts to factor
ll$Species <- factor(ll$Species)
ll$Site <- factor(ll$Site)
ll$Location <- factor(ll$Location)

ll$Date.Num <- factor(ll$Date, levels = c("03/20/2024","03/12/2024", "03/17/2024"),
                    labels = c("One", "Two", "Three"))

#### model ####
ll1 <-glmmTMB(RB.sec ~ Site + (1|Location), data = ll)

ll0 <-glmmTMB(RB.sec ~ 1 + (1|Location), data = ll)

##  compare models 
anova(ll1,ll0)

#### post-hoc ####
emmeans(ll1, list(pairwise ~ Site), adjust = "tukey")
